...
export default Klass.extend({
  ...
  setupContext: function() {
    ...
    setContext({
      ...
      dispatcher: null,
      register: function() {
        let target = this.registry || this.container;
    ...
    if (Ember.inject) {
      let keys = (Object.keys || Ember.keys)(Ember.inject);
  ...
  contextualizeCallbacks: function() {
    ...
    let keys = (Object.keys || Ember.keys)(callbacks);
  ...
  _contextualizeCallback: function(context, key) {
    ...
    context[key] = function(options) {
      ...
      let result = callbacks[key].call(_this, options, factory());
  ...
  _setupContainer: function(isolated) {
    ...
    let items = buildRegistry(!isolated ? resolver : Object.create(resolver, {
      resolve: {
        value: function() {}
      }
    }));
    ...
    if (hasEmberVersion(1, 13)) {
      ...
      let router = resolver.resolve('router:main');
  ...
  _setupIsolatedContainer: function() {
    ...
    for (let i = this.needs.length; i > 0; i--) {
      let fullName = this.needs[i - 1];
      let normalizedFullName = resolver.normalize(fullName);
